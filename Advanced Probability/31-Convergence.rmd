...
2.  As $\lim_{n \to \infty}\mathop{{}\mathbb{P}}\{\left|X_n-X\right|>\epsilon\}=0$, we can select a subsequence s.t.
    $$
...
For every subsequence $N\subset \mathbb{N}^*$, $X_n \xrightarrow{p} X$ along which and theorem \@ref(thm:convergence-probability).2 implies $N'\subset N$ exists s.t. $X_n \to X$ a.s. along and thus $f(X_n)\to X$ a.s. along $N'$. It follows that $f(X_n)\xrightarrow{p} f(X)$ by theorem \@ref(thm:convergence-probability).3.
...
$c\implies a$. Note $X_n^{p} \xrightarrow{p} X^{p}$ by proposition \@ref(prp:convergence-probability-continuous).
...
$$
thus $X^{p} \in L^{1}$. Let $F_n=\{\left|X_n^{p}-X^{p}\right|^{}>\epsilon\}$, note
$$
\begin{aligned}
    \mathop{{}\mathbb{E}}\left|X_n-X\right|^{p}&\le \mathop{{}\mathbb{E}}\left|X_n^{p}-X^{p}\right| 
    \\&\le \epsilon+\mathop{{}\mathbb{E}}\left|X_n^{p}-X^{p}\right|\bm{\mathbf{1}}_{F_n}
\end{aligned}
$$
As $X^{p}$ is integrable and $X_n^{p}$ is uniformly so, $(X_n^{p}-X^{p})$ is uniformly integrable and thus the right side can be arbitrary small if $\mathop{{}\mathbb{P}}(F_n)$ can be also arbitrary small. It follows that $\mathop{{}\mathbb{E}} \left|X_n -X\right|^{p} \to 0$.
