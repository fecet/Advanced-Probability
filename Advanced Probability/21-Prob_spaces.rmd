...
1. $r$th Moment: $\mathbb{E}X^r$
2. $r$th Absolute Moment: $\mathbb{E}|X|^r$
3. $r$th Central Moment: $\mathbb{E}(X-\mathbb{E}X)^r$
4. $r$th Absolute Central Moment: $\mathbb{E}|X-\mathbb{E}X|^r$
...
1. $r \mapsto \hat{\mu}_r$ is continues and decreasing on $(0,\infty)$ and note $e^{-rX}=e^{-rX}\bm{\mathbf{1}}_{X<\infty}\nearrow \bm{\mathbf{1}}_{X<\infty}$ as $r \searrow 0$, then $\lim_{r \to 0^+}\hat{\mu}_r=\mathbb{P}\{X<\infty\}$
2.  $\hat{\mu}_r$ is also called **Moment generating function** as
    $$
    \lim_{r \to 0^+}\frac{d^n}{dr^n} \hat{\mu}_r=(-1)^{n}\mathbb{E}X^{n}
    $$
    if $\mathbb{E}X^{n}<\infty$
...
Similarly, we have
$$
\lim_{r \to 0^+}\frac{d^n}{dr^n} \hat{\mu}_r=i^{n}\mathbb{E}X^{n}
$$
if $\mathbb{E}X^{n}<\infty$
...
$$ \mathbb{E} | X Y | \leq \left[ \mathbb{E} | X | ^ { p } \right] ^ { 1 / p } \left[ \mathbb{E} | Y | ^ { q } \right] ^ { 1 / q } $$

:::

Suppose $r>1$,
$$
\|XY\|_r=\left( \mathbb{E}|X^{r}Y^{r}| \right)^{\frac{1}{r}}\le (\mathbb{E}|X^{r}|^{p})^{\frac{1}{pr}} (\mathbb{E}|X^{r}|^{q})^{\frac{1}{qr}}=\|X\|_{rp}\|Y\|_{rq}
$$
that implies
...
$$ \mathbb{E} | X Y | \leq \sqrt { \left[ \mathbb{E} | X | ^ { 2 } \right] \left[ \mathbb{E} | Y | ^ { 2 } \right] } $$

:::

::: {.theorem #lyapunov name="Lyapunov's inequality"}

1. $\forall p\ge 1, \mathbb{E}|X|\le \mathbb{E}(|X|^p)^{\frac{1}{p}}$
2. $\forall 0<r\le s<\infty,\left[ \mathbb{E} | Z | ^ { r } \right] ^ { 1 / r } \leq \left[ \mathbb{E} | Z | ^ { s } \right] ^ { 1 / s }$
...
$\forall p\ge 1$,
$$ (\mathbb{E}|\sum X_i|^p)^{\frac{1}{p}}\le \sum (\mathbb{E}|X_i|^p)^{\frac{1}{p}} $$
...
::: {.theorem #Markov name="Chebyshev(Markov) inequality"}

If $g$ is strictly increasing and positive on $\Reals_+$, $g(x)=g(-x)$, and $X$ is a r.v. s.t. $\mathbb{E}(g(X))<\infty$, then $\forall a>0$
$$
...
$$
\lim_{b \to \infty} \mathbb{E} |X|\bm{\mathbf{1}}_{|X|>b}=0
$$
...
$\implies$ is follows from theorem \@ref(thm:DCT) as $|X|\bm{\mathbf{1}}_{|X|>b}\searrow 0$. Conversely, taking $b=c\gg 1$ s.t. $\mathbb{E}|X|\bm{\mathbf{1}}_{|X|>c}\le 1$ and then
$$
\mathbb{E}|X|\le \mathbb{E}(c+|X|\bm{\mathbf{1}}_{|X|>c})\le c+1<\infty
$$
...
$$
k(b)=\sup_{X\in \mathcal{K}} \mathbb{E}|X|\bm{\mathbf{1}}_{|X|>b} \to 0
$$
as $b \to \infty$.
...
3.  Uniform integrability implies $L^1$-boundedness: $\mathcal{K}\subset L^1$ and $\sup_{\mathcal{K}} \mathbb{E} |X|<\infty$. That follows from
    $$
    \begin{aligned}
        \mathbb{E}|X|&\le \mathbb{E}\left( b+\mathbb{E}X \bm{\mathbf{1}}_{|X|>b} \right)
        \\ &=
        b+\mathbb{E}X \bm{\mathbf{1}}_{|X|>b}
        \\&\le
        b+ k(b)
    \end{aligned}
    $$
    holds for each $X\in \mathcal{K}$.
4. $L^1$ boundedness is not sufficient for uniform integrability.
5. $L^{p}$ boundedness when $p>1$ implies uniform integrability.
...
$$
\mathbb{P}(F)\le \delta\implies \sup_{X\in \mathcal{K}} \mathbb{E}_F|X|\le \epsilon
$$
...
$$
\sup_{X\in \mathcal{K}} \mathbb{E} X \bm{\mathbf{1}}_{F}\le b \mathbb{P}(F)+k(b)
$$
then $\implies$ is immediately as $k(b)$ can be arbitrary small.

Conversely, by Markov's inequality \@ref(thm:Markov):
$$
\sup_{X\in \mathcal{K}} \mathbb{P}\{X>b\}\le \frac{1}{b} \sup_{X\in \mathcal{K}} \mathbb{E}X=\frac{k(0)}{b}
$$
that suggests we may choose $b$ s.t. $\mathbb{P}\{X>b\}$ arbitrary small, and thus $\sup \mathbb{E}_F X$ arbitrary small, taking $H=\{X>b\}$, then we have definition of uniformly integrability exactly.
