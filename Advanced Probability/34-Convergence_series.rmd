...
Let $\mathop{{}\mathbb{E}}Z_i=\mu_i$ and $\mathop{\text{Var}}Z_i=\sigma_i^2$, suppose $\mu_i\to \mu$ and $\sigma_i\to \sigma \neq 0$, each $\bm{\mathbf{X}}_{ij}$ is bounded by $c_{ij}$ and $\lim_{i \to \infty}\sup _{j}c_{ij}=0$. Then, $\frac{Z_i-a}{b} \xrightarrow{d}\mathfrak{Z}$.
...
$$
where $\epsilon_i \to 0$ by assumption and thus theorem \@ref(thm:liapunov-theorem) applies to $\bm{\mathbf{Y}}_{ij}$ to show:
$$
...
In view of theorem \@ref(thm:distribution-quantile-convergence), then claim is that
$$
\lim_{n \to \infty}\mathop{{}\mathbb{P}}\{\frac{S_n-n\mu}{\sqrt{n\sigma^2}}\le x\}=\int_{-\infty}^{x} \frac{e^{-\frac{y^2}{2}}}{\sqrt{2\pi}}dy
$$
