...
$$
\begin{aligned}
    \pi(A\times B)&=\pi \bm{\mathbf{1}}_{A\times B}
    \\ &= 
    \int_{X} \int_{Y} \bm{\mathbf{1}}_{A\times B}(x,y) d\nu d\mu
    \\ &= 
    \int_{X} \int_{Y} \bm{\mathbf{1}}_{A}(x) \bm{\mathbf{1}}_B(y) d\nu d\mu
    \\ &= \mu(A)\nu(B)
\end{aligned}
$$
...
Suppose there is countable $f_i:(X,\mathcal{F})\to (\Omega_i,\mathcal{A_i})$ for $i \in I$ and define $f(x)=(f_i(x))_{i \in I}$, then $f$ is measurable iff each $f_i$ is measurable.
