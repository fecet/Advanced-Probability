...
Suppose $\mathbf{r}$ is a $n\times 1$ random vector with mean $\overline{\mathbf{r}}$ and invertible variance $\mathbf{V}$, then the portfolio return is
...
**Proof** A portofolio $\omega$ is on the efficient froniter iff it's return $\overline{r}_p\ge r_{mv}$ and with the  form $\omega=a+b \overline{r}_p$, thus there is a bijection between $\omega_i$ and $r_i$. Suppose the return of $\omega_i$ is $r_i$, 
...
$$ \bm{\mathbf{e'(\Omega c)=(e'\Omega)c=e'c}=1} $$

since $\forall \omega_i \mathbf{e'\omega_i}=1, \mathbf{e'c}=1$. By above theorem, the combaination is on the MVF. Then it's sufficient to show that $\mathbf{c'r} \ge r_{mv}$. It's clearly since
...
$$ \text{Var}(\mathbf{r_p})=\overbrace{\underbrace{\sigma_{mv}^2}_{\text{unavoidable risk}}+b_p^2\text{Var}(\mathbf{r^*})}^{\text{systematic risk}}+\underbrace{\text{Var}(\epsilon_p)}_{\text{idio risk}} $$
...
That is the tangency portofolio. If $\overline{r}_q>0$, $\overline{r}_q$ can be interpreted as risk-free asset return.
...
1. 
   The maximun sharpe ratio is the slope of frontier $\sqrt{(\mathbf{\overline{r}}-r_f\mathbf{e})'\mathbf{V}^-(\mathbf{\overline{r}}-r_f\mathbf{e})}$. One can check that

   $$ \sqrt{(\mathbf{\overline{r}}-r_f\mathbf{e})'\mathbf{V}^-(\mathbf{\overline{r}}-r_f\mathbf{e})}=\frac{\omega'(\mathbf{\overline{r}}-r_f\mathbf{e})}{\sqrt{\omega'\mathbf{V}\omega}}$$

   when $\bm{\mathbf{\mathbf{V^-}(\overline{\mathbf{r}}-r_f\mathbf{e})}}=\mathbf{V^-}(\overline{\mathbf{r}}-r_f\mathbf{e})$.
