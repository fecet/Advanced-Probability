...
$\implies$, let $f$ be the functional in theorem \@ref(thm:SHT), note $M$ is a linear space, $f$ should be vanish on $M$, $i.e.$, $f(x)=0,\forall x \in M$, otherwise, fix $f(y)>0$ for $y\in K-\left\{ 0 \right\}$, we can find $\lambda \in \mathbb{R}$ $s.t.$ $\lambda f(x)=f(\lambda x)>f(y)$.
